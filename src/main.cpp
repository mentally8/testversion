...
// Copyright (c) 2009-2012 The Vaultcoin developers
...
static const int HALFWAY_BLOCK = 1440;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    unsigned int nProofOfWorkLimit = bnTargetLimit.GetCompact();
    
    int64_t retargetTimespan = BLOCK_TIME;
    ...
    if ((pindexLast->nHeight+1) % retargetInterval != 0)
    {
        if (fTestNet)
        {
            ...
            if (pindexLast->GetBlockTime() > pindexLast->GetBlockTime() + BLOCK_TIME*2)
                return nProofOfWorkLimit;
            else
            {
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal vaultcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            uint64 nCoinAge;
            if (!GetCoinAge(txdb, nCoinAge, pindexBlock->nHeight))
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Vaultcoin
    bool fStrictPayToScriptHash = true; // Always active in Vaultcoin
    ...
    // ppcoin: fees are not collected by miners as in vaultcoin
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge, const int nHeight) const
{
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Vaultcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeTargetSpacing = 90; // test block spacing is 1.5 minutes
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "VaultCoin is the best long-term store of wealth!";
        CTransaction txNew;
        txNew.nTime = 1406411046;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406411046;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 65824;

        if(fTestNet)
        {
            block.nTime    = 1406413229;
            block.nNonce   = 2365301;
        }
        ...
        assert(block.hashMerkleRoot == uint256("f90f1279420624ce1d22f9f67f54327c4411c41502224145c96b78659ab367c7"));
...
unsigned char pchMessageStart[4] = { 0x80, 0x35, 0x22, 0x05 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("VaultcoinMiner : proof-of-work not meeting target");
...
void static ThreadVaultcoinMiner(void* parg);

static bool fGenerateVaultcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void VaultcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateVaultcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateVaultcoins) && !fProofOfStake)
                return;
        }
        ...
        loop
        {
            ...
            if (fShutdown)
                return;
            if (!fGenerateVaultcoins)
                return;
            if (fLimitProcessors && vnThreadsRunning[THREAD_MINER] > nLimitProcessors)
                return;
            if (vNodes.empty())
                break;
            if (nBlockNonce >= max_nonce)
                break;
...
void static ThreadVaultcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        VaultcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadVaultcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadVaultcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadVaultcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateVaultcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateVaultcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateVaultcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d VaultcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadVaultcoinMiner, pwallet))
                printf("Error: NewThread(ThreadVaultcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
